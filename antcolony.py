...
class AntColony:
	class Ant(Thread):
		def __init__(
				self, init_location, possible_locations, pheromone_map,
			...
			possible_locations -> a list of possible nodes the ant can go to
				when used internally, gives a list of possible locations the ant can traverse
				to _minus those nodes already visited_
			...
			alpha -> a parameter from the ACO algorithm to control the influence of the amount
				of pheromone when making a choice in _pick_path()
			...
			pheromone_trail -> a list of pheromone amounts deposited along the ants trail,
				maps to each traversal in route
		...
		def run(self):
			...
			while self.possible_locations:
				_next = self._pick_path()
				self._traverse(self.location, _next)
	...
	def __init__(
			self, nodes, distance_callback, start=None, ant_count=50, alpha=.5, beta=1.2,
			pheromone_evaporation_coefficient=.40, pheromone_constant=1000.0, iterations=80):
		"""
		initializes an ant colony (houses a number of worker ants that will traverse a map to
		find an optimal route as per ACO [Ant Colony Optimization])
		...
		pheromone_map -> holds final values of pheromones
			...
			pheromone dissipation happens to these values first, before adding pheromone values
			from the ants during their traversal
		...
		alpha -> a parameter from the ACO algorithm to control the influence of
			the amount of pheromone when an ant makes a choice
		...
		pheromone_evaporation_coefficient -> a parameter used in removing pheromone values from the
			pheromone_map (rho in ACO algorithm)
			used by _update_pheromone_map()
		...
		# before adding those to pheromone_map during the update_pheromone_map step
	...
	def _get_distance(self, start, end):
		...
		if not self.distance_matrix[start][end]:
			...
			if (type(distance) is not int) and (type(distance) is not float) and (type(distance) is not np.float64):
				raise TypeError("distance_callback should return either int or float, saw: {}".format(str(type(distance))))
	...
	def _init_nodes(nodes):
		...
		for key in sorted(nodes.keys()):
			id_to_key[_id] = key
			id_to_values[_id] = nodes[key]
			_id += 1
			
		return id_to_key, id_to_values
		
	@staticmethod
	def _init_matrix(size, value=0.0):
		"""
	...
	def _init_ants(self, start):
		...
		if self.first_pass:
			return [
				self.Ant(
					start, list(self.nodes.keys()), self.pheromone_map, self._get_distance,
					self.alpha, self.beta, first_pass=True)
	...
	def mainloop(self):
		...
		for iteration in range(self.iterations):
			...
			for ant in self.ants:	
				# update ant_updated_pheromone_map with this ant's contribution of pheromones along its route
			...
			# decay current pheromone values and add all pheromone values we saw during traversal (from
			...
			self.it_best.append(self.shortest_distance)
		# translate shortest path back into callers node id's
		ret = []
		for _id in self.shortest_path_seen:
			ret.append(self.id_to_key[_id])
